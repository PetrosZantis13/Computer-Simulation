...
class CelestialBody(object):
    ...
    def updatePos(self):  
        ...
        self.position = self.position + self.velocity*self.dt + (1/6)*(4 * self.accelerations[1] - self.accelerations[0] )* ((self.dt)**2)
        
    def updateVel(self):    
             
        self.velocity = self.velocity + (self.dt/6)*(2*self.accelerations[2]  + 5*self.accelerations[1] - self.accelerations[0])
